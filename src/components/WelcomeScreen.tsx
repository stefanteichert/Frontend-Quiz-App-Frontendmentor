...
const WelcomeScreen = ({ quizzes, onSelect }: WelcomeScreenProps) => {
  return (
    <section className="flex flex-col gap-10 desktop:flex-row desktop:gap-32" aria-label="Welcome">
      <div className="w-full flex flex-col gap-4 desktop:gap-12 desktop:max-w-116.25  desktop:pb-4">
        <h1 className="flex flex-col gap-2">
          <span aria-hidden="true" className="text-preset-2-light text-blue-900 dark:text-white">Welcome to the</span>
          <span aria-hidden="true" className="text-preset-2-medium text-blue-900 dark:text-white">Frontend Quiz!</span>
          <span className="sr-only">Welcome to the Frontend Quiz!</span>
        </h1>
      ...
      <div className="flex flex-col gap-4 desktop:flex-1">
        {quizzes.map((quiz) => {
          const quizTheme = getTopicTheme(quiz.title);
          return (
            <button
              type="button"
              key={quiz.title}
