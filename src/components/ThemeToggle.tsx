...
const ThemeToggle = ({ theme, onToggle }: ThemeToggleProps) => {
  return (
    ...
    <label htmlFor="theme-switch" className="flex items-center gap-2 tablet:gap-4 cursor-pointer">
      ...
      <span className="relative min-w-8 min-h-5 tablet:w-12 tablet:h-7">
        <input
          id="theme-switch"
          type="checkbox"
        ...
        {/* Track */}
        <span className="absolute inset-0 rounded-full bg-purple-600 transition-colors duration-300 p-1" />

        {/* Thumb */}
        <span className="absolute left-1 top-1 w-3 h-3 tablet:w-5 tablet:h-5 bg-white rounded-full transition-all duration-300 peer-checked:translate-x-3 tablet:peer-checked:translate-x-5" />
      </span>
